<template>
  ...
  >
    ...
    <div class="wen-crud__body">
      ...
      <!-- 可展开的 -->
        ...
        >
          ...
          <template slot-scope="scope">
            <div class="cell-wrapper" @contextmenu.prevent="handleRowContextMenu(scope)">
          ...
          <template v-if="item.children">
            <mutli-column v-for="(child, idx) in (item.children || [])" :key="idx" :item="child" />
          </template>
        </el-table-column>
        ...
        >
          ...
          <template slot-scope="scope">
            ...
            <template v-if="showUpdate || updateContent">
              ...
              <el-button  v-else type="primary" size="mini" @click="openUpdateDialog(scope)">
                编辑
              </el-button>
            </template>
          </template>
      ...
      >
        <el-form ref="form" :model="dialogFormData" :rules="rules" label-width="100px">
          <el-row>
        ...
        <div slot="footer">
          <el-button @click="onDialogCancel">{{ dialogOptions.cancelText || '取消' }}</el-button>
          <el-button type="primary" @click="onDialogSave">{{ dialogOptions.confirmText || '确认' }}</el-button>
        </div>
      </el-dialog>
    </div>
  </div>
